...
class GitlabCommit < ApplicationRecord
  ...
  has_many :diffs, class_name: 'GitlabCommitDiff', dependent: :destroy, inverse_of: :gitlab_commit
  ...
  def format_diff_snippets(diffs, total_diff_count)
    records = Array(diffs.presence || self.diffs.limit(5)).reject(&:blank?)
    return if records.empty?

    lines = records.map(&:embed_snippet)
    total = total_diff_count || (self.diffs.loaded? ? self.diffs.size : self.diffs.count)
